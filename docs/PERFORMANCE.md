...
cold prompt
  ...
  -> optional patch + optional cached/fresh rerender
...
| Asset validation | Concurrent reads with in-run checksum de-duplication |
...
| No-op QA | No second scene revision and no second render |
...
The current loop has fixed phases, one conditional branch, and at most one correction. An orchestration framework would add state serialization, adapters, and debugging layers without shortening this code.
...
Adopt a durable engine only when at least one real requirement appears: multi-worker ownership, crash resume inside a phase, human approval waits, scheduled jobs, or loops beyond the single bounded correction.
...
The next spatial upgrade should be mesh-derived bounds and depth/segmentation evidence from Blender or Three.js, not a generic graph database.
